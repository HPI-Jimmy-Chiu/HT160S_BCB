...
- `.claude/skills/weekly-case-flow/SKILL.md` — weekly report + customer-case flow (Hub mode). HT160S progress + KYEC(京元竹南) anomaly cases go into the SHARED store `D:\Work-jimmychiu\document\WeeklyReport\Weekly_AI` (used by HT9045/HT172 too). Driven by `.claude/agents/weekly-report.md` + `.claude/commands/weekly-*.md`. NOTE: release-note/close_case tools there are HT9045-specific; HT160S uses ht160s-installer/NSIS instead.
  **Case granularity (Weekly_AI ADR-008, 2026-09-15): KYEC HT160S is a PRODUCTION machine (3 units). Every customer anomaly = its own weekly row + its own `CASE-KYEC_CHEN-YYYYMMDD-NNN` with HT160S-specific fields (`--serial` KYEC-0N / `--component` HT160S vocabulary / `--tags HT160S,<module>,KYEC` / `--version 部署YYYYMMDD`). The former 「機台開發」 row is now 「HT160S 量產維護（非客訴開發紀錄）」 and takes non-complaint work only — never append complaints to it.**
