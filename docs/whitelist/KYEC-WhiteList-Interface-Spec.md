...
| 對象 | 客戶 EAP / IT 介接端 |
| 版本 | v1.3（2026-08-04）—— 僅一項變更：§2.4 的 SECS 模式回讀（SVID 66032）已取消，設定端（§2.2 / §2.3 的 LOTSTART + SORTMODE）與白名單檔案格式（§3）完全未變 |
...
### 2.4 模式回讀（⚠ 2026-08-04 起已取消）

> ⚠️ **本節的 SECS 回讀能力已於 2026-08-04 移除。**
>
> 原本主機可用 `S1F3` 查詢 SVID `66032` 讀回有效分選模式。依客戶「HT-160S 只公佈 HT-90XX 家族編號」
> 的裁定，本機自訂的 66xxx 段（含 66032）已**全數下架**，而 HT-90XX 家族**沒有**分選模式的對應編號
> （家族中唯一近似的 35530「[I27] Manual sort mode」是 HT9045/46 的 BOOLEAN 機台選項，型別與語意皆不合），
> 因此**沒有替代的 SVID**。
>
> **設定端完全未變**：主機仍以 `LOTSTART` 的 `SORTMODE` 選項對切換白名單模式（§2.2、§2.3 全部有效，
> `HCACK=0` 即代表模式切換成功）。改變的只是「回讀」——主機無法再用 `S1F3` 確認目前模式。
> 若查詢已下架的 66032，`S1F4` 該格會回**空清單** `<L[0]>`（清單長度不變，可與「已註冊但無值」的
> `<A[0]>` 區分），不是錯誤碼。
>
> **替代確認方式**：看機台端 —— 維護畫面的 Sort Mode 選擇器與白名單面板的啟用狀態，或 Process 記錄中
> 的 `WhiteList: ...` 行。若貴端的流程必須有 SECS 回讀，請配發一個 HT-90XX 家族編號給本方實作。
>
> 以下原有的值域說明保留供理解機台行為之用（機台的模式判定邏輯完全未變，只是不再對外公佈編號）：

原先主機以 `S1F3` 查詢 SVID `66032`（INT4），回覆目前**有效**分選模式（基礎模式 + 白名單覆蓋合併後的結果）：
...
- 白名單 Lot 進行中為 `3`；Lot End 自動還原後，回到該機台的基礎模式（`0`、`1` 或 `2`）。⚠ 這仍是機台內部的行為，只是自 2026-08-04 起無法由 SECS 讀出。
...
4. （選用）確認模式已切換：`LOTSTART` 回 `HCACK=0` 即代表切換成功。⚠ 2026-08-04 起**不再能**以 `S1F3` 查詢 SVID 66032 回讀（見 §2.4）；需要目視確認時請看機台維護畫面的白名單面板。
5. Lot End 後，機台自動還原為基礎分選模式（`0`／`1`／`2`，視現場 Sort Mode 設定）。

> **「白名單模式已切換成功」不代表白名單檔案已被接受。** 兩者是分開的：
> 模式由 `LOTSTART` 的 `SORTMODE` 決定（`HCACK=0` 即成功），檔案則在 Lot Start 當下才讀取與檢查。
> 檔案若不合格，模式仍是白名單，但機台名單為空、生產 Start 會被擋下。檔案是否被接受，
> 請看機台端的提示訊息或 Process 記錄中的 `WhiteList: ...` 行。
> ⚠ 這一點在 2026-08-04 移除 66032 回讀後更重要：模式切換成功與檔案有效**都不能**靠 SECS 回讀確認。
