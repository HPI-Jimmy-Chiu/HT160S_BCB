...
本章說明 HT160S 的「產品設定 (Product Setup)」畫面，以及與之相關的配方 (Recipe / WorkFile) 管理、料盤幾何 (Tray Form)、料區對 Bin 的對應 (Bin Area Map)，並涵蓋兩種分流模式：靜態的 Normal 模式與動態的 By Lot+Bin 模式。設定畫面以「配方」為單位儲存，所有資料寫入 `data\<配方名>` 資料夾下的 `setup.ini`、`BinAreaMap.ini`、`manifest.ini`。
...
> ⚠️ 注意：在 By Lot+Bin 模式 (`GeneralSetting.bUseLotBinSortMode`=true) 下，整張表的 Bin 欄禁止手動編輯（Auto↔Bin 由執行時動態綁定），此時只能修改 `Error Bin`。
...
| `GeneralSetting.bUseLotBinSortMode` | bool | 是否為 By Lot+Bin 動態排序模式（此畫面唯讀使用，於 GeneralSetting 設定） |

---

## 6.6 分流模式：Normal 與 By Lot+Bin

HT160S 把分類結果 (Bin) 導向輸出料倉（Auto1~Auto6 / Color / Error 區）有兩種模式：
...
模式切換與多數硬體/路由旗標的設定畫面位於 maintenance 硬體安裝頁 (`tsMaintHardware`)，本節一併說明。
...
| 控制項 | 類型 | 功能 |
| --- | --- | --- |
| `chkUseLotBinMode` | checkbox | 切換分流模式 Normal ↔ By Lot+Bin（寫入 `GeneralSetting.bUseLotBinSortMode` / `General.ini [SortMode] UseLotBinMode`）；變更後僅提醒重開軟體，不強制；lot 執行中被鎖定 |
| `chkAutoEnable1..6` | checkbox | 逐一啟用/停用 Auto1~Auto6 輸出站（`bAutoEnabled[0..5]` / `[SortMode] AutoEnabled0..5`）；停用者於綁定新 (LotID,Bin) 時被跳過；變更後提醒重開；lot 執行中鎖定 |
...
### 6.6.4 啟用 By Lot+Bin 動態分流模式

1. 於 maintenance 硬體頁勾選 `chkUseLotBinMode`（`bUseLotBinSortMode`=true），依提示重開軟體乾淨生效。
2. 以 `chkAutoEnable1..6` 設定哪些 Auto 可用於分流（停用者於綁定時被跳過）。
3. 載入各 Lot 的 2D/Bin 資料（離線匯入或 WebAPI/SECS），使 `LotRegistry.GetItemCount()`>0。
4. 按 Start 前系統檢查：By Lot+Bin 模式必須已存在至少一筆 (Lot,Bin)→Auto 綁定，否則擋下並提示 `By Lot+Bin mode is ON but no binding is set. Set bindings first !`。

### 6.6.5 By Lot+Bin 執行期綁定與放料

1. Loader CCD 掃描每顆 IC：`LotRegistry.FindByCode2D`(2D 碼) 反查出所屬 LotIndex 與 Bin，寫入該料格（`SetTrayBin` / `SetTrayLot` / `SetTrayCode2D`）。
2. 若 `bUseLotBinSortMode`：呼叫 `LotBinBinding.ResolveAuto(LotIndex, Bin)`。先查既有綁定，無則以「先到先得」選最低 index、未被佔用且 `bAutoEnabled` 為真的非 Error Auto；全被佔用時溢位到 Error Auto。新綁定立即 `AddObject` 並 `SaveToIni()`。
3. SortArm 放料時 `GetMappedAutoIndex` 只 READ 綁定（`FindAuto`）無配置副作用；LotIndex<0 或錯誤 Bin（2D 掃描失敗/無 Bin 設定）導向 Error Auto；此模式不使用 Color 區做分流。
...
- **硬體編輯鎖 (ApplyHardwareEditLock)**：Lot 執行中 (`MachineRun.bRunning`) 鎖定 `chkUseLotBinMode` / `chkAutoEnable1..6` / `chkSuckEnable1..4` / `chkHardwareColorBinArea` / `chkUseAMR`，避免中途改路由破壞進行中的分流；重啟（未起 lot）即解鎖，標題加註 locked。
...
| `By Lot+Bin mode is ON but no binding is set. Set bindings first !` | By Lot+Bin 模式開啟但尚無任何綁定 | 掃描產生綁定或先設定綁定後再 Start |
...
- 【待補】畫面中文標籤：設定畫面可見文字皆為英文，未出現 Big5 中文標籤；硬體頁部分 checkbox 標籤（`chkUseLotBinMode`、`chkAutoEnable*`、`chkSuckEnable*`、`chkHardwareColorBinArea`、`chkUseAMR`、`cbBinPanelType`、`edLotNo`）實際螢幕 Caption 需以 .dfm 或執行畫面確認。
