...
> ⚠️⚠️ **下表是 2026-07-29 之前的舊 CEID 字典，號碼與意義都已作廢，請勿據以設定主機。**
> 2026-07-29 起本機的 CEID 字典整份改為 HT-90XX 的逐字複本（號碼 1–292，別名逐字相同），**同號不同義**：例如舊表的 1=HandlerStatus / 2=RecipeChange / 3=ClearCount / 4=按 Start 無 IC，現行為 **1=Start Pressed（機內無 IC）／2=Pause Pressed／3=OneCycle Pressed／4=CleanOut Pressed**，而「機內有 IC 時按 Start」是 **76**、「機台狀態變更」是 **27**、「按清除計數」是 **5**。照舊表設定會被靜默誤讀（訊息格式合法、語意錯誤）。
> **權威來源請看** `docs/SECS/SECS_GEM功能_Handler_20260804.xlsx` 的 CEID 工作表（292 筆，含本機是否真的會發射）與 `docs/SECS/HT160S_SECS_Interface_Spec_20260727.md` §3.3（含新舊對照表 §3.3.5）。本表待整份重寫，暫留供辨識舊設定之用。
...
> 註（2026-08-04 起，SVID 對齊 HT-90XX 家族）：依客戶裁定，本機僅公佈 HT-90XX 家族號，原 HT160 自訂的 66000／66001／66002／66010／66011／66020／66021／66030／66031／66032 共 10 個 SVID **全部下架**（註冊數 76→67）。查詢已下架的號碼時，S1F4 該格回**空清單** `<L[0]>`（可與「已註冊但目前無值」的空字串 `<A[0]>` 區分），清單長度不變。替代讀法：動作模式→1011（或動作 CEID）；有無在跑→1011 與 Start／Pause 事件（⚠ **Start 有兩個號碼**：CEID 1 只在機內無 IC 時發，機內有 IC 時的 Start 是 **CEID 76**——暫停後續跑、警報解除後續跑都走 76，現場這才是常態；停止側是 CEID 2）；控制狀態→SVID 4（前一狀態 9）；IC 計數→1101 / 1102（1102 原本就與舊 66021 綁同一顆計數器）；Lot 批數→**1006 為空字串時是 0 批**，否則為逗號數 +1（⚠ 必須先判空：未開批時 1006 回主畫面 Lot 欄位的文字，那是 0 批，直接算「逗號數 +1」會把 0 批誤報成 1 批）；第一批 Lot 編號→1006 第一個逗號前的字串；警報→S5F1 / S5F6。**已知且經客戶接受的代價：分選模式（Sort Mode）家族無對應 SVID，主機仍可用 S2F41 `LOTSTART` 的 `SORTMODE` 設定，但無法再以 S1F3 讀回。**

> 註（預設事件報告 Report 1 = 單一項目）：Report 1 的內容為 **`{1027 System Time}` 一個 SV**，與 HT9045 及客戶 HT9046 的持久化定義逐位元相同。292 個 CEID 中有 288 個連到 Report 1，故絕大多數 S6F11 只帶一個時戳值（形如 `L[3]{DATAID, CEID, L[1]{L[2]{RPTID 1, L[1]{<A 19 碼時戳>}}}}`）；S1F24 事件名單同鏈，該 288 筆各只列 1 個 VID。Report 2–7（AMR 位元圖、各站盤數／IC 數、身分盤 2D）完全不變。與 KYEC 主機對接的最大架構差異：9045 的報告為 host-dynamic（S2F33/35/37 動態定義），HT160 出廠先定義 7 個設備端 report（Report 1–7）作為基準。⚠ **2026-08-04 更正**：原文寫「HT160 為 equipment-static 固定 7 個 report」並不正確 —— 本機**有**實作主機端動態報表定義（S2F33/F34 定義、S2F35/F36 連結、S2F37/F38 啟用），且**報表定義會存檔**（`system\EventReportDef.ini`，開機載入）。真正不持久化的只有 **S2F35 的「事件→報表」連結**，這正是下面那條復電提醒的成因。完整對照見 `docs/AGV/HT9045_vs_HT160_SECS_Diff_20260625.md`、現況介面合約見 `docs/SECS/HT160S_SECS_Comm_Examples.md`。
...
> 註（CEID 跳號）：Auto1–6 卸盤完成事件為 **{136,137,138,145,146,147}**——**139 在 9045 CEID 空間已被占用**（`DoVisualSortLotStart`），HT160 為避免撞號刻意跳過。⚠ **本段兩處已於 2026-08-04 更正**：(1) Auto4–6 的號碼**不是** 140/141/142 —— 那組是 2026-07 前的舊編號，已改為 **145/146/147**（140 與 9045 的 Prepare Load 撞號，而 141 自 2026-08-03 起被重新指派為 GEM Control State Change，是活躍事件、與卸盤無關）；(2) 這 6 個 CEID **有掛 report** —— 韌體把 292 個 CEID 全部連到 Report 1，所以它們現在帶一格 `1027 System Time`，**不是空 body**（2026-07-29 之前才是空的）。另 Auto 車滿預告事件沿用 9045 保留號：Auto1–3=35/36/37、Auto4–6=148/149/150（掛 Report 1，與 CEID272 同時雙發）。
