...
- **By Lot+PassFail**：分類鍵 = (LotID, PASS/FAIL)，PASS/FAIL 由「客戶 2D 資料中每顆 IC 的 DiePass（1=PASS／0=FAIL）」導出（機台端無須任何設定），一個 Lot 最多綁 2 個 Auto（PASS 一個、FAIL 一個）。
...
| By Lot+PassFail（動態） | 掃描時依每顆 IC 的 DiePass 分為 PASS（DiePass=1）或 FAIL（DiePass=0 或無此資料），依先到先得把每組 (Lot 編號, PASS/FAIL) 綁定到 Auto，之後僅讀取此綁定放料 | 維護畫面硬體安裝頁的 **Sort Mode** 選擇框；PASS／FAIL 來源為載入的 2D 資料，綁定會存回機台設定檔 |
...
- **By Lot+PassFail**：只需把每個 Lot 的良品（DiePass=1）與非良品分成兩倉時。每 Lot 最多佔 2 個 Auto，適合良/不良二分的收料需求。
...
分流模式切換與各 Auto 啟用，位於維護畫面的硬體安裝頁（Hardware Setup）的 **Sort Mode** 選擇框；By Lot+PassFail 不需要在機台上設定 PASS／FAIL——它直接讀載入的 2D 資料中每顆 IC 的 DiePass。Lot 編號與工單清單則在主畫面的 Lot 分頁。

![Lot 分頁與分流設定](screenshots/main-lot.png)
> 圖 15-1 主畫面 Lot 分頁與分流設定。（擷取方式：開啟主畫面，切換至 Lot 分頁；模式選擇位於維護畫面硬體安裝頁的 Sort Mode 選擇框）
...
| PASS／FAIL 來源 | 無機台設定；來自載入的 2D 資料 | 每顆 IC 的 `DiePass`（客戶 WebAPI 提供，1=PASS／0=FAIL）。無此資料者一律視為 FAIL。同時決定 Production_Log 的 PassFail 欄 |
...
By Lot+PassFail 把每顆 IC 依 2D 資料中的 `DiePass` 分成 PASS / FAIL 兩類，各自綁一個 Auto。機台端沒有任何 PASS／FAIL 設定要做。

1. 進入維護畫面硬體安裝頁，在 **Sort Mode** 選擇框選 **By Lot+PassFail**，依提示重開軟體。
2. 以 **Auto1**～**Auto6** 啟用勾選設定可用 Auto（每 Lot 最多用到 2 個，建議至少保留 2 個非 Error Auto 啟用）。
3. 載入各 Lot 的 2D／Bin 資料（同 15.5 步驟 5）。這份資料的 `DiePass` 就是 PASS／FAIL 的唯一依據。
4. 按 **Start**（前置檢查同 15.5，沒有額外的 PASS／FAIL 設定檢查）。
5. 運轉後：CCD 掃描每顆 IC 時，以該顆的 `DiePass` 判定 PASS／FAIL，**在掃描當下凍結**此分類到料格（不在放料時重算），並依先到先得綁定 (Lot 編號, PASS/FAIL)→Auto。

> ⚠️ 注意（無 DiePass 資料＝FAIL）：若某顆 IC 的 2D 資料沒有帶 `DiePass`（例如手動輸入的 2D／Bin、或舊格式資料），一律判為 **FAIL**，不會被丟到 Error Auto。若整批資料都沒有 DiePass，整批會全部進同一個 FAIL Auto——請確認 2D 資料是由客戶 WebAPI 拉取的。
>
> ⚠️ 說明（分類凍結）：PASS／FAIL 來自可被重新拉取／更新的 2D 資料，與 Bin（IC 内生、不變）不同，因此在掃描當下凍結於料格，放料時只讀取、不重算，確保掃描時與放料時算出同一個結果。
>
...
1. Loader CCD 掃描每顆 IC：以 2D 碼反查出所屬 Lot 與 Bin，寫入該料格。同時依該顆的 DiePass 計算 PASS／FAIL 分類並凍結。
2. 依模式綁定：
   - **By Lot+Bin**：以 (Lot, Bin) 取得對應 Auto。
   - **By Lot+PassFail**：分類為 PASS／FAIL 時以 (Lot, 分類) 取得對應 Auto；若為錯誤 Bin（2D 掃描失敗／無 Bin 設定）則不綁定，放料時導向 Error Auto。
...
1. 開機：載入設定（分流模式、各項功能旗標）→ 載入目前配方與其 Bin→Auto 對應表 → 載入盤面規格。
2. 主畫面：載入累計統計；還原上次的 Lot 清單與綁定 → 若有內容則彈出「是否繼承上次工單」提示（Yes 續跑／No 清空）。
3. 資料載入：每顆 IC 以 (Lot 編號, 2D 碼, Bin) 登錄；2D 碼全域唯一，重複者會被拒絕、不覆蓋。
4. Start 前置檢查：需有 Lot 編號、有已載入的 2D 資料、且目前 Lot 有可路由的 2D／Bin（By Lot+PassFail 無額外檢查）。
