...
argument-hint: "例如：力成PTI 反應 HT172 異常，如附圖"
---
...
6. **客戶名稱鎖定**：只用使用者原話逐字的客戶名稱。勿把 HT9045/HT160S 語境的客戶帶進來。委派時 prompt 裡只能出現 Step 0 解析出的精確客戶名稱。

## 流程
0. 客戶名稱解析（精確比對，不可省略）：`cd tools && python _customers.py "<使用者原話客戶名稱逐字>"` → 取得 `RESOLVED` 正式名稱才能往下；`UNKNOWN` 先跟使用者確認，不可自行猜測代換。
1. 解析：機型（HT172）、版本、問題、日期、附件描述（客戶名稱已在 Step 0 鎖定）。
2. 查既有：`weekly_data.json`（僅用 Step 0 鎖定的正式客戶名稱 + HT172 + 關鍵字/版本/CASE-ID）與 `Customer/<客戶>/`。
3. **先說明**將新增或重啟哪一筆、要建哪個 case 資料夾，經我同意後：
   ...
   # 有原始檔：
   python archive_issue.py <最終row> "<source_file>" --desc "<短描述>" --expect-customer "<客戶>"
   # 尚無檔案（只有圖片描述）：
   python archive_issue.py <最終row> --skeleton-only --desc "<短描述>" --expect-customer "<客戶>"
   ```
...
> 後續我把異常檔放進 `01_intake/` 並說「開始分析」→ 交回 **HT172 開發 session 本體**做根因分析（遵守 Big5 + FSM 慣例；不另設分析子代理）。
