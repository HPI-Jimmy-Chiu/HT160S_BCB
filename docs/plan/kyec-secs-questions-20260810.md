...
> **狀態（2026-08-10 更新）**：Q1 **已改 code 並上線**（commit `b51e2b2`，見 §1.6）；
> Q2 / Q3 / Q4 仍只做分析，**未改任何 code**，待 §5 裁決。
...
### 2.6 本項與貴端 2026-08-04「只用家族號」裁示直接衝突

這是本次四問裡我方**唯一無法自行消化**的一點，必須請貴端拍板，因為它牴觸的是**貴端自己六天前的裁示**。

**2026-08-04 的裁示是什麼**（記錄在 `SecsGem/uHGemHT160.cpp:650-691` 的 tombstone）：

> 「HT-160S 只能發布**家族（HT-90XX）號碼**」——貴端據此逐號檢視 `66000-66039` 這整個
> **HT-160S 專屬**號段，把全部 10 個號當場下架，並要求號碼**永久保留、絕不重用**。
> 理由是：讀未註冊號碼會回空 `L[0]`，是**可偵測**的；一旦重用，舊 host 的殘留綁定
> 就會從「可偵測的空白」變成「無法偵測的錯值」。

**Q2 與這條裁示的三處衝突**：

| # | 2026-08-04 裁示 | Q2 的要求 | 衝突 |
|---|---|---|---|
| 1 | 只用**家族既有**號碼 | 指定 `38237/38238/38239` | 這三個號**在我方 11 個 HT-90XX 版本全文搜尋為零**（§2.2）。它們不是家族號，是**新造號**——而新造號正是 66xxx 被下架的原因。 |
| 2 | 家族有同義號就必須改用同義號 | 未提及 `37003-37005` | 家族**已經有**語意完全吻合的 `UnloaderTrayCount_ART_Auto1/2/3`（§2.3），而貴端 host 目前**完全沒引用**。照 0804 的規則，這裡應該直接改用 37003-37005。 |
| 3 | 號碼一經指派/退役即**永不重用**，以免「可偵測的空白」變成「無法偵測的錯值」 | 要求把已在用的 38237-38239 改成別的語意 | 這三個號 HT-160S **已經在用**（AMR Auto4-6 Tray Count），而且已寫進 2026-08-02 交付貴端的規格書。重新定義語意**正是** 0804 裁示要防的那種情況：舊 host 讀到的不是空白，而是**看起來合理但意義完全不同的數字**。 |

**我方立場**：貴端 0804 的原則我方認為是對的，也已照辦（66xxx 全數下架）。
若 Q2 要推翻它，我方照辦，但需要貴端明確授權；否則我方建議**照 0804 原則走 §5 Q2-b**，
也就是**直接改用家族既有的 37003-37005**——這樣三處衝突同時消失，也不必動任何既有號碼。
...
| **Q2-f** | Q2 與貴端 **2026-08-04「只用家族號、退役號永不重用」**裁示直接牴觸（§2.6 三處）。請確認是否推翻該裁示？ | 66xxx 整段就是依該裁示下架的。若要新造 38237-38239，等於推翻自己的規則；照原規則走 Q2-b（改用 37003-37005）則三處衝突同時消失。 |
