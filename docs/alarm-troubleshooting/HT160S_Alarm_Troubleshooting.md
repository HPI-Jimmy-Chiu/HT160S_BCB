...
- **可能原因**：先前發生故障、被中止,或斷電重開之後,有一個盤子被留在 Color 後方交接位置,沒有清走。；後方偵測盤子的感測器(SnColor_OutputBottomHasTray)卡在一直有訊號(一直亮)的狀態,其實沒有盤子卻被誤判成有盤。；先前的接收/回收盤子動作被中途打斷,系統內部記著「後方有盤」,還沒有重新用感測器實況更新過來。
- **裝置狀態**：
...
- **通報工程師**：若後方交接位置實際上已經沒有盤子,警報卻一直出現,請通報工程師,並提供以下資訊:在 IOsetview(IO 監看畫面)查看 SnColor_OutputBottomHasTray 這顆後方感測器,是否卡在一直亮(ON)的狀態,並確認它是「啟用」狀態(卡在亮會讓系統誤判後方有盤)。同時請工程師調閱狀態紀錄,確認先前是否有被中途打斷的接收/回收盤子動作,導致系統內部「後方有盤」的記憶沒有更新回來。
