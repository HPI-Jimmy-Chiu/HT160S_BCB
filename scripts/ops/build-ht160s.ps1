...
# AI(ht160s-buildstamp) 20260910 : stamp the real build identity into BuildInfo.h for the
# duration of this build, then put the committed placeholder back in the finally below.
# WHY: both KYEC machines reported "HT160S 1.0.0.0" on 2026-09-09 while machine 2 ran a
# demonstrably older exe, so no on-site log could be tied to a known code state. Computed
# BEFORE the file is written, or the dirty check would always see its own edit. Declared
# out here (not inside the try) because Set-StrictMode 2.0 makes an unassigned variable in
# the finally a hard error, and because the restore MUST run even if a check throws.
$buildInfoPath = Join-Path $projectRoot "BuildInfo.h"
$buildInfoSaved = $null
if (Test-Path -LiteralPath $buildInfoPath) {
    $buildInfoSaved = [System.IO.File]::ReadAllBytes($buildInfoPath)

    $gitSha = "nogit"
    try {
        $shaOut = & git -C $repoRoot rev-parse --short HEAD 2>$null
        if ($shaOut) { $gitSha = ([string](@($shaOut)[0])).Trim() }
    } catch { $gitSha = "nogit" }
    $dirtyMark = ""
    try {
        $dirtyOut = & git -C $repoRoot status --porcelain -- "HT160S_Program_BCB_V1.0.0.0" 2>$null
        if ($dirtyOut) { $dirtyMark = "+" }
    } catch { $dirtyMark = "" }
    $buildStamp = "$gitSha$dirtyMark " + (Get-Date -Format "yyyyMMdd-HHmm")

    # LF endings + pure ASCII + no BOM, to match the committed placeholder and keep
    # check-ht160s-source-encoding.ps1 happy.
    $buildInfoLines = @(
        "//---------------------------------------------------------------------------",
        "// BuildInfo.h - GENERATED by scripts/ops/build-ht160s.ps1 for this build only.",
        "// The committed placeholder is restored when the build finishes. Do not edit.",
        "//---------------------------------------------------------------------------",
        "#ifndef BuildInfoH",
        "#define BuildInfoH",
        "#define HT160S_BUILD_ID `"$buildStamp`"",
        "#endif",
        "//---------------------------------------------------------------------------"
    )
    [System.IO.File]::WriteAllText($buildInfoPath, (($buildInfoLines -join "`n") + "`n"), (New-Object System.Text.ASCIIEncoding))
    Write-Host "Build stamp: $buildStamp"
}
else {
    Write-Warning "BuildInfo.h is missing - the exe will carry the cmydef.h fallback build id."
}
...
try {
    ...
    # AI(ht160s-buildstamp) 20260910 : put the committed placeholder back, so the working
    # tree never carries a machine-specific stamp and git stays clean after a build.
    if ($null -ne $buildInfoSaved) {
        [System.IO.File]::WriteAllBytes($buildInfoPath, $buildInfoSaved)
    }
    Pop-Location
}
